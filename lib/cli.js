...
(async () => {
  ...
  // delBran.remoteTrackingBranches(remote).forEach((h, b) => {
  //   console.log(`${h} ${b}`);
  // });

  const rtbs = delBran.remoteTrackingBranches(remote);
  ...
  if (rtbs.size && program.notOpen) {
    const openPRBranches = await delBran.openPRRefNames(token, remoteUrl, 200);
